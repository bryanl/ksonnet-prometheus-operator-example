...

{
  ...
  kubeStateMetrics+:: {
    ...
    service:
      ...
      local ksmServicePortMain = servicePort.new().withPort(8443).withTargetPort('https-main') +
      + service.mixin.metadata.withName('https-main');
      local ksmServicePortSelf = servicePort.new().withPort(9443).withTargetPort('https-self') +
      + service.mixin.metadata.withName('https-self');
